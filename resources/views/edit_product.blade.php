        ...
        <form action="{{route('update_product',$product->id)}}" method="post">
            ...
            <div class="row row-cols-md-4 row-cols-sm-3 row-cols-2 mb-4">
                <div class="col px-3 py-2">
                    ...
                    <input value="{{$product->name}}" type="text" name="name" id="name" class="form-control form-control-sm @error('name') is-invalid @enderror form-field" placeholder="e.g. full slive t-shirt" requid>
                ...
                <div class="col px-3 py-2">
                    ...
                    <select name="catagory" id="catagory" class="form-control form-control-sm @error('catagory') is-invalid @enderror " requid>
                        ...
                        @foreach($catagories as $catagory)
                            <option value="{{$catagory->id}}" {{$product->catagory==$catagory->id?'selected':''}}>{{$catagory->name}}</option>
                        @endforeach
                    </select>
                    ...
                    <select name="sub_catagory" id="sub_catagory" class="form-control form-control-sm @error('sub_catagory') is-invalid @enderror " requid>
                        <option selected disabled>Select Sub-catagory</option>
                            @foreach($sub_catagories as $sub_catagory)
                                <option value="{{$sub_catagory->id}}" {{$product->sub_catagory==$sub_catagory->id?'selected':''}}>{{$sub_catagory->name}}</option>
                            @endforeach
                    </select>
                    ...
                    <tbody id="form_tbody">
                        @foreach($variants as $variant)
                            <tr  id="row_{{$variant->id}}">
                                <td>
                                    <select name="color[]" id="" class="form-control form-control-sm color" requid>
                                        ...
                                        @foreach($colors as $color)
                                            <option value="{{$color->id}}" {{$color->id==$variant->color?'selected':''}} >{{$color->name}}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="number" value="{{$variant->quantity}}" name="quantity[]" id="" class="form-control form-control-sm quantity integer-only positive-num-only" placeholder="e.g. 10" requid>
                                </td>
                                <td>
                                    <input type="number" value="{{$variant->price}}" name="price[]" step="0.01" id="" class="form-control form-control-sm price positive-num-only" placeholder="e.g. 10.00" requid>
                                </td>
                                <td>
                                    <input type="number" value="{{$variant->selling_price}}" name="selling_price[]" step="0.01" id="" class="form-control form-control-sm selling_price positive-num-only" placeholder="e.g. 10.00" requid>
                                </td>
                                <td>
                                    <input type="number" value="{{$variant->discount_amount}}" name="discount_amount[]" step="0.01" id="" class="form-control form-control-sm discount_amount positive-num-only" placeholder="e.g. 10.00" requid>
                                </td>
                                <td>
                                    <button type="button" class="remove-row btn text-danger" data-row-id="row_{{$variant->id}}"><span class="material-symbols-outlined">disabled_by_default</span></button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
